...
Qwen3.8-27B was then tested, not merely shortlisted. With a 19.68 GB Q5_K_S GGUF and 0.93 GB BF16 vision projector on the RTX 5090, llama.cpp b9733 recorded 21,161 MiB used after load. The original non-speculative profile delivered about 64.2 tok/s across five tests; enabling the model's embedded MTP head at depth 2 raised that mean to 100.9 tok/s (+57.2%), with 92.7–113.1 tok/s across native text, constrained Chinese, constrained Python, native tool calling, and vision. All gates passed. Cold loading from D: through WSL 9p remains slow, so it is a strong exclusive-window planner/reviewer rather than a model to reload between every video shot. Raw evidence and the reproducible launcher are in the [Qwen3.8 experiment repository](https://github.com/WHYKEYSAY/serve-qwen3.8-27b).
